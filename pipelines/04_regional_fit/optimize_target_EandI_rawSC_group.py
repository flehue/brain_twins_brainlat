"""Optimize DMF targets for the raw-SC group analysis.

Self-contained Omat-only version. One group per run, selected by the
1-based row index in optimals_BOLD_corr_by_group_rawSC.csv.
"""
...
GROUPS_PATH = SCRIPT_DIR.parent / "03_global_fit" / "optimals_BOLD_corr_by_group_rawSC.csv"
INPUT_ROOT = Path("/data/workspaces/neuromodelling/rherzog/Brainlat/BOLD/optimization/input")
EMPIRICAL_GROUPS_PATH = INPUT_ROOT / "average_oinfo_matrices_by_group.csv"
RAW_SC_PATH = INPUT_ROOT / "raw_SC.npz"
OUTPUT_BASE = Path("/data/workspaces/neuromodelling/rherzog/Brainlat/BOLD/optimization/optimize_target_from_omat_epsilon0.04/rawSC/group")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optimize DMF targets for a single raw-SC group")
    parser.add_argument("row_index", type=int, help="1-based row index in optimals_BOLD_corr_by_group_rawSC.csv")
...
def resolve_group(row_index_1_based: int) -> GroupParams:
    groups = pd.read_csv(GROUPS_PATH)
    idx = row_index_1_based - 1
    if not 0 <= idx < len(groups):
        raise IndexError(f"Row index {row_index_1_based} is out of bounds for {len(groups)} groups.")

    row = groups.iloc[idx]
    dataset = str(row["Dataset"]).strip()
    diagnosis = str(row["Diagnosis"]).strip().upper()
    sex = str(row["Sex"]).strip()
    age_range = str(row["Age_Range"]).strip()
    group_key = f"{dataset}_{diagnosis}_{sex}_{age_range}"
...
def load_group_data(params: GroupParams) -> Tuple[np.ndarray, np.ndarray]:
    ...
    omat_df = pd.read_csv(EMPIRICAL_GROUPS_PATH)
    mask = (
        omat_df["Dataset"].astype(str).str.strip().eq(params.dataset)
        & omat_df["Diagnosis"].astype(str).str.strip().str.upper().eq(params.diagnosis)
        & omat_df["Sex"].astype(str).str.strip().eq(params.sex)
        & omat_df["Age_Range"].astype(str).str.strip().eq(params.age_range)
    )
    if not mask.any():
        sample = ", ".join(
            f"{row.Dataset}_{str(row.Diagnosis).upper()}_{row.Sex}_{row.Age_Range}"
            for row in omat_df.head(10).itertuples(index=False)
        )
        raise KeyError(f"Group key {params.group_key} not found in {EMPIRICAL_GROUPS_PATH}. Sample keys: {sample}")
    link_cols = [col for col in omat_df.columns if col.startswith("link_")]
    empirical_omats = omat_df.loc[mask, link_cols].iloc[0].to_numpy(dtype=np.float64)
...
def ensure_output_structure(output_root: Path) -> Dict[str, Path]:
    ...
    output = folder / "output_per_group"
    for path in (plots, bold, output):
        path.mkdir(parents=True, exist_ok=True)

    master = folder / "master_file_rawSC_group.csv"
    if not master.exists():
        roi_cols = "\t".join(f"ROI_{n}" for n in range(NNODES))
        master.write_text(
            "GROUP_ID\tDataset\tDiagnosis\tSex\tAge_Range\tG_base\ttarget_base\ttime_taken\tmean_entropy_E\tmean_entropy_I\tmean_omat_emp\t"
            + roi_cols + "\n"
        )
    return {"plots": plots, "bold": bold, "output": output, "master": master}


def plot_results(plot_path: Path, params: GroupParams, corrs: np.ndarray, all_targets: np.ndarray,
    ...
    fig.tight_layout()
    fig.savefig(plot_path / f"{params.group_key}.png")
    plt.close(fig)


def write_master_line(master_path: Path, params: GroupParams, base_target: float, elapsed: float,
    ...
    with master_path.open("a") as handle:
        handle.write(
            f"{params.group_key}\t{params.dataset}\t{params.diagnosis}\t{params.sex}\t{params.age_range}\t"
            f"{params.g_value}\t{base_target}\t{elapsed}\t{mean_entropy_e}\t"
            f"{mean_entropy_i}\t{mean_omat_emp}\t" + "\t".join(map(str, optimal_targets)) + "\n"
        )


def optimise_group(params: GroupParams, max_iters: int, patience: int) -> None:
    structural_connectivity, empirical_omats_flat = load_group_data(params)
    ...
    best_corr = -np.inf
    no_improve = 0
    bold_output = output["bold"] / f"BOLD_optimal_group{params.group_key}.npz"

    start = tm()
    for iteration in range(max_iters):
        all_targets[:, iteration] = target_vector
        agg_omat = np.zeros((NNODES, NNODES), dtype=np.float64)
        ...
        for seed in range(NSEEDS):
            ...
            agg_omat += np.asarray(sim_omat, dtype=np.float64)
            agg_mean_e += mean_e
            agg_mean_i += mean_i
            agg_entropy_e += ent_e
            agg_entropy_i += ent_i
            bold_to_save[f"seed{seed}_array"] = bold
            print(f"GROUP={params.group_key} iter={iteration + 1}/{max_iters} seed={seed} done", flush=True)
        ...
        print(
            f"GROUP={params.group_key} iter={iteration + 1}/{max_iters} corr={corr:.6f} seeds_time={tm() - iter_start:.2f}s",
            flush=True,
        )
    ...
    np.savez_compressed(
        output["output"] / f"group{params.group_key}_output.npz",
...
def main() -> None:
    args = parse_args()
    params = resolve_group(args.row_index)
    optimise_group(params, max_iters=args.max_iters, patience=args.patience)
