...
def Sim(verbose = False, return_rates = False):
    ...
    Y_t_rates_E = np.zeros((len(timeSim),nnodes)) #Matrix to save values (firing rates)
    Y_t_rates_I = np.zeros((len(timeSim),nnodes)) #Matrix to save values (firing rates)
    
    for i in range(0,Nsim):
        Y_t_rates_E[i,:],Y_t_rates_I[i,:] = rE,rI
        derivs, rE,rI = mean_field(neural_Var, SC, params, target)
        neural_Var += derivs * dt + Noise(sigma) * np.sqrt(dt)
                
    return Y_t_rates_E,Y_t_rates_I, timeSim
